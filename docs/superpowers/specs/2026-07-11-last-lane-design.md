...
- Vercel Blob stores two deterministic objects per verified result with `addRandomSuffix: false`. `runs/{nonce}.json` is first created with `allowOverwrite: false` in a `pending` state and contains the complete derived result. `scores/{invertedScore20}/{invertedSurvivalTicks10}/{submittedAt}/{nonce}.json` is an immutable lexicographically sortable projection, where fixed-width inverted numeric fields make Blob's pathname order equal the leaderboard order. After projection creation, the run object transitions to `published` using its ETag and `ifMatch`. The read API lists only the first 100 score objects and fetches those bounded objects; the initial release displays the top 100 runs and does not scan the archive or compact it.
- If `runs/{nonce}.json` already exists, submission reads it and returns success only when its ticket digest and derived result match; a conflicting reuse is rejected. A matching pending run may be repaired even after the ticket deadline because its transcript was already verified before the deadline. A bounded scheduled reconciliation lists at most 100 pending run objects per invocation, creates any missing deterministic score projections, and conditionally marks them published. Pending objects older than 24 hours are retained for audit but no longer retried automatically. Tests inject failure after the pending write with no immediate client retry and prove reconciliation publishes it.

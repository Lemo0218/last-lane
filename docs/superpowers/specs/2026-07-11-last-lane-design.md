...
Ranked play starts by requesting a server-issued run ticket containing a random server seed, one-time nonce, issue time, twelve-minute submission deadline, and simulation ruleset version, authenticated with a server-only HMAC. The client cannot choose ranked seeds. Verification derives duration from transcript ticks rather than client wall time. Each nonce creates at most one result, making retries idempotent and replays harmless.
...
- Vercel Blob uses three deterministic path prefixes with `addRandomSuffix: false`. `pending/{nonce}.json` is first created with `allowOverwrite: false` and contains the complete derived result. `scores/{invertedScore20}/{invertedSurvivalTicks10}/{submittedAt}/{nonce}.json` is an immutable lexicographically sortable projection, where fixed-width inverted numeric fields make Blob's pathname order equal the leaderboard order. After projection creation, the same immutable result is copied to `runs/{nonce}.json`, then the pending work item is deleted. The read API lists only the first 100 score objects and fetches those bounded objects; the initial release displays the top 100 runs and does not scan the archive or compact it.
- Submission checks both `runs/{nonce}.json` and `pending/{nonce}.json`, returning success only when the ticket digest and derived result match; a conflicting reuse is rejected. A matching pending run may be repaired even after the ticket deadline because its transcript was already verified before the deadline. A scheduled reconciliation lists at most 100 objects directly from the `pending/` prefix, creates missing deterministic score and audit objects, then deletes the work item. Pending objects older than 24 hours move to `failed/{nonce}.json` for audit and are no longer retried automatically. Tests inject failure after the pending write with no immediate client retry and prove reconciliation publishes it even when more than 100 published runs exist.
